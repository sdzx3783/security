$(function(){
	var subform={
			...
			/**
			 * 处理页面多值的控件类型 如select checkbox radio,把字段信息放到那些组件的val属性中
			 * eg:<select name="m:zxdmsckjcs:zbzdw" val="${zxdmsckjcs.zbzdw}"><option value="1">苹果</option> <option value="2">HTC</option> </select>
			 */
			initMulValCtrl:function(){
				...
				//处理checkbox
				$("input[type=checkbox][val]").each(function(){
					var data = $(this).attr("val");
			...
			addBind: function( table) {
				...
				if($(table).attr('id')=='weekPlanSbb'){
					//周计划 不要binding
				}else{
					$('[type="subdata"]:not(".forbidevent")').live("contextmenu", function(e) {
