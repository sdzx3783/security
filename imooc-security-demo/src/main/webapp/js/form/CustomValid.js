...
(function($) {
	$.extend($.fn, {
		...
		valid : function(conf) {
			...
			//	遍历有validate属性的元素
			$('[validate]:visible', form).each(function() {
				//	returnObj 中success为false的为验证不通过的结果
			...
			// _v变量
			// 只要有一个returnObj.success为false，那么_v的值则为false,全部通过为true

			// 手动调用表单校验时,conf为真，并且需要错误信息返回
			if(conf&&conf.returnErrorMsg){
				...
				if(!_v){
					...
					for(var m in msgs){
						...
						if(m=="_elem"){
							// 获取错误标题
							if($(msgs[m]).parents('tr').find('th').length > 0) errorTitle = $(msgs[m]).parents('tr').find('th').text().replace(/\*/, '') || "";
							else errorTitle = $(msgs[m]).parents('td').prev('td, th').text().replace(/\*/, '') || "";
							continue;
						}else{
							
							errorMsg = errorMsg +"<span style='color:rgb(136, 10, 10);font-weight: bolder;'>"+
							errorTitle+"&nbsp&nbsp</span> <span style='color:red;'>"+msgs[m]+"</span></br>";
						}
		...
		handValidResult : function(obj) {
			...
			//PS：上面那是WDR的逻辑，但是在附件必填时会报错（因为附件没数据时就是一个a按钮），
			//为了不破坏原来逻辑（虽然我不懂为啥要这样做），只能在其基础上添加判断了
			if($(obj).is("a")&&!$(obj).is(".selectFile.link")){
				return;
			}
		...
		getValue : function(element){
			...
			if(element.is(":checkbox,:radio")) {
				...
				value = element.parents("td").find("input[name='"+textboxname+"']").val();
				if(!value){
					value = element.val();
				}
			}else {
		...
		rules : [{
					...
					name : "number",
					rule : function(v) {
						var res = /^-?((\d{1,3}(,\d{3})+?|\d+)(\.\d{1,6})?)$/.test(v.trim());
						return res;
					},
					...
					name : "maxLength",
					rule : function(v, b) {
						return (v.trim().replace(/[^x00-xFF]/g,'**').length <= b);
					},
