...
jQuery.extend({
			 ...
			 * @returns
			 */
			openFullWindowOld:function(url){
...
// 这里就是让各个页面table数据列表下，分页条固定定位的代码
/**
...
function changeScrollHeight(noNeedFoldHeight, time){
	window.setTimeout(function(){
		...
		//	如果当前页面不是处于main.ht下，则取得#searchForm的高度，一般情况
		//	下是处于main.ht下的，所以这个一般都不会执行进去
		if(tableBody.length>0){
			if(!parent.document.querySelectorAll("#logoImg")[0]){
				foldHeight = $("#searchForm").height();
			}
			//	通常情况下panel-search上没有配置moreHeight属性，此处为0
			var moreHeight = $('.panel-search').attr("moreHeight") || 0;
			//	开始给table的容器设置高度
			var finalHeight = $(window).height() - tableBody[0].offsetTop - $(".index_menu", $(parent.document)).height() / 2 - $("#oa_list_paging").height() - 10;
			tableBody.height(finalHeight);
