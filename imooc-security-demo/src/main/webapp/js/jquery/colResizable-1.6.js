...
(function($){ 	
	...
	var createGrips = function(t){	
		...
		th.each(function(i){						//iterate through the table column headers			
            ...
            var dc = t.dc.length>0?(t.dc.indexOf(i)!=-1):false;           //is this a disabled column?
	...
	*/
	var syncCols = function(t,i,isOver){
		var inc = drag.x-drag.l, c = t.c[i], c2 = t.c[i+1],cLast = t.c[t.ln-1]; 			
		var w = c.w + inc;	var w2= c2.w- inc;	wLast = cLast.w - inc;//their new width is obtained	
		
		var tabOverFlowWidth = $('.fixed-table-container').width() - 38 - t.width();
        ...
        if(t.f){ //if fixed mode
            ...
            if(tabOverFlowWidth > 0 && inc < 0){
            	cLast.width(wLast + PX);
            	t.css('width', $('.fixed-table-container').width() - 38);
            } else {
            	t.css('width', t.w + inc);
            }
        }
		if(isOver){
            c.w=w; 
            c2.w= t.f ? w2 : c2.w;
            cLast.w= (tabOverFlowWidth > 0 && inc < 0 ) ? wLast : cLast.w;
        }
	...
	*/
	...
	var onGripDrag = function(e){	
		...
		var tabOverFlowWidth = $('.fixed-table-container').width() - 22 - (t.w + x - drag.l);
		if(tabOverFlowWidth > 0){
			if(last){
				x = x + tabOverFlowWidth;
			}
		}
