...
(function($) {
	...
	$.extend($.PaginationRenderers.defaultRenderer.prototype, {
		...
		getLinks : function(current_page, eventHandler) {
			...
			var eachSize = "<select class='eachPageSize' >" +
			"<option value='10' "+(this.opts.items_per_page==10?'selected':'') + ">10</option>" +
			"<option value='20' "+(this.opts.items_per_page==20?'selected':'') + ">20</option>" +
			"<option value='50' "+(this.opts.items_per_page==50?'selected':'') + ">50</option>" +
			"<option value='100' "+(this.opts.items_per_page==100?'selected':'') + ">100</option>" +
			"</select>";
			
			var begin, end, interval = this.pc.getInterval(current_page), np = this.pc.numPages(), fragment = $("<div class='pagination'><span style='color: black;'>第 <em>"
					+ (current_page + 1) + "</em> 页/共 <em>" + np + "</em> 页 每页 <em>" + this.opts.items_per_page + "</em> 条记录 共 <em>" + this.maxentries + "</em> 条记录  " + eachSize + " </span></div>");

			if (this.opts.showtext && this.opts.showtext == 2)
				fragment = $("<div class='pagination'><span style='color: black;'>共 <em>" + this.maxentries + "</em> 条记录</span></div>");
	...
	$.fn.pagination = function(maxentries, opts) {
		...
		$(".eachPageSize").val(opts.items_per_page);
		$(document).on("change",".eachPageSize",function(){  
			opts.callbackMain();
        })  
