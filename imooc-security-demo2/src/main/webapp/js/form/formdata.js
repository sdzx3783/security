(function($) {
	$.extend($.fn, {
		...
		/**
		 * 处理字段名称，现在字段名称为: 主表： m:maintablename:fieldname 子表：
		 * s:subtablename:fieldname 变成单纯的 fieldname
		 */
		...
		handleFieldNameReduction : function(head) {
			form = $(this).find(".formTable");
			form.find('[name]').each(function() {
				var obj = $(this);
				var name = obj.attr('name');
				obj.attr('name', head + ":" + name);
			});
		},
